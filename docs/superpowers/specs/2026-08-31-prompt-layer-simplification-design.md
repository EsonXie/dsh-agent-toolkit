...
1. **identity 层**——dsh 原生 `harness:identity`（order -100），只读展示。
2. **模型识别层**——按模型命中内置模型族提示词，UI 只读。
3. **persona 层**——Agent 性格/角色/职责，**唯一可编辑层**。
...
1. **模型层 UI 只读**：模型族提示词内置（`DEFAULT_RULES` + cordis.yml rules 追加），UI 面板只读展示；可编辑的只有 persona。
...
|---|---------|-------|---------|-----------|
| 1 | `harness:identity` | -100 | 只读 | 原生（现状不变） |
...
- `prompt_layers` 表单行结构不变（`{ layers: LayerConfig[] }`）。
- `openLayerSource` reconcile 天然迁移：种子单层化后，已存的 base/domain/task 层连同文本直接丢弃，persona 文本保留（order 以种子为准 = 10）。
- `validateFixedLayers` 种子 = 单 persona 层；`LayerSource.set` 仅允许改 persona 文本，UI 与 API 均无法增删层。
...
- `PUT`：`source.set` 固定结构校验（种子单层）不符 → 400，同现有通道。
- `POST /reset`：不变（回种子单层）。

## 第 6 节 · 浏览器半 UI（`src/client/prompt/PromptLayersModal.tsx`）

- 层列表（硬编码顺序）：`harness:identity`（只读徽标）→ **模型层**（只读，展示 `modelFallbackText` + 说明「按模型命中规则覆盖，见下方规则区」）→ **persona**（可编辑 textarea）→ `model-notes`（只读徽标）→ 动态层折叠区（contexts 快照，现状不变）。
- 删除 base/domain/task 编辑行；「重置为默认层」「规则（只读）」保留。
- `api.ts`：`fetchPromptLayers` 返回类型扩展 `modelFallbackText`。
...
- Node 半：
  ...
  - `prompt/index` 相关：base 固定注册（order 0，规则覆盖生效）；persona 注册为普通段（order 10）；无 `deployment:persona` 改写（cordis.yml persona 原样通过）；子 Agent 隔离；钉住与 model-notes 不变。
