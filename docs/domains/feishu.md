...
运维指令面（0.3.1 起）：`/new` `/stop` `/status` 为整条精确匹配指令；`/sessions` 列出 bot 项目 workspace 候选会话（workspaceRegistry 候选 + live 会话标题，标题取不到显示 (无标题)，三服务缺席降级）；`/switch <序号|id前缀>` 把 chat 绑定覆盖到目标会话（序号指最近一次 /sessions 输出的 per-chat 内存缓存；目标在插件内存直接复用、在宿主内存存活（web 界面等持有写句柄）则接管复用（保持其当前装配，setup 不重跑）、冷会话才 resume 接管（套用 bot 装配）；binding 在接管/resume 成功后才覆盖；切走的旧 runtime 不 retire，在飞 turn 卡片照常收尾，闲置落定且未重新绑定后摘出 sessions）；`/help` 列出全部指令；workspaceRegistry 缺席时 /sessions 与 /switch 回复「会话切换在当前环境不可用」。已知限制：/switch 目标会话若正绑定在其他 chat（其 runtime 仍在内存），当前实现直接复用该 runtime——审批卡与在飞卡片仍发往原 chat，且原 chat 的 /new 会取消该会话任务；请避免跨 chat 切换同一会话。摘除 runtime（retire / releaseUnbound / 覆盖失败清理）一律 dispose 宿主 AgentHandle 释放写句柄（接管来的 live 会话 dispose 为空操作，句柄归原主），否则自有会话永远 already owned 无法再 resume；live 检查与 resume 之间的竞态（恰好被他人打开）兜底回复「该会话正被占用（可能在 web 界面打开中），请关闭后重试」，绑定不变。
