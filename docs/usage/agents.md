...
> 截图待补拍：设置面板收编后 Agents 页为卡片流（原「左侧角色列表 + 右侧编辑器」弹窗截图已过时）。

## 打开面板

打开宿主**设置面板 → 「Agent 工具箱」**（页内三 tab：Agents / 定时任务 / 分层提示词），默认即 Agents 页。页面为**卡片流**：按创建时间升序排列，`main`（主 Agent）恒置顶且只读（无编辑/删除按钮，仅显示「使用宿主默认模型与装配」），内置角色带「内置」徽标且不显示删除按钮。每张卡片下方是该 Agent 名下的飞书 Bot 列表（含「+ 添加 Bot」按钮，归属锁定到本卡）。点「编辑」在卡片内**内联展开**编辑器；点顶部「新建角色」内联展开新建表单。保存/删除成功显示 toast。
...
> 编辑器截图待补拍：模型选择与工具白名单区块沿用原布局，现已内联在卡片内（explorer 回显默认只读白名单）。
