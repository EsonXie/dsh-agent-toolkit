...
宿主**设置面板 → 「Agent 工具箱」→ 定时任务**：整页任务行列表，每行显示名称、cron 人类可读描述（如「每天 09:00」）、执行目标、启用开关与下次触发时间；**点击行展开运行历史**。行内操作：编辑、立即触发、删除（两段确认）；启用开关**行内即时生效**并弹 toast。「+ 添加任务」内联展开创建表单（cron 表达式实时预览未来 3 次触发）。
