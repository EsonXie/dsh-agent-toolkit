...
`refresh` 用于修复统计缺失（例如多实例并行时后到实例不计量导致的漏记）：它重新扫描全部会话日志，对范围内每一天**整体重建并替换**已有记录；范围内已有记录但日志中已无对应会话事件的日期会被删除。天数取 1..366，缺省 30。仅计量主实例可执行；扫描瞬间在途未落盘的调用可能被覆盖，再刷一次即可收敛。个别会话日志读取失败时会记 warn 并跳过该会话：可读的日期照常重建，但本次**不会执行任何删除**（避免把读失败的日期误删），修复日志后再刷即可。
