...
**话题群**：群聊开启话题模式后，会话按（bot, 群聊, 话题）三元绑定——同一话题群的每个话题都有独立会话，互不共享上下文；在某个话题内 @ 机器人，回复（含审批卡、问答卡）落在该话题内，不会新开话题。`/new`、`/stop`、`/status`、`/sessions`、`/switch` 与消息排队都只作用于当前话题。升级说明：升级前在话题群建立过的（bot, 群聊）级绑定不再命中（保留但不再使用），在话题内重新 @ 机器人即建立该话题的会话。
