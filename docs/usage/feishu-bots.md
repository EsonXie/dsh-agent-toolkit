...
![Bot 列表并入 Agents 页各 Agent 卡片（连接状态点 + 编辑/删除）](images/agents-cards.png)
...
![创建向导第 1 步：基本信息（内联在 Agent 卡片内，无「绑定 Agent」字段）](images/bots-form.png)
...
**话题群**：群聊开启话题模式后，会话按（bot, 群聊, 话题）三元绑定——同一话题群的每个话题都有独立会话，互不共享上下文；在某个话题内 @ 机器人，回复（含审批卡、问答卡）落在该话题内，不会新开话题。`/new`、`/stop`、`/status`、`/sessions`、`/switch` 与消息排队都只作用于当前话题。注意 `/switch` 的候选列表仍是该 bot 项目 workspace 的全量会话（含其他话题的绑定会话），本期不限制跨话题切换。升级说明：升级前在话题群建立过的（bot, 群聊）级绑定不再命中（保留但不再使用），在话题内重新 @ 机器人即建立该话题的会话。
...
- 存储域 `project_bot`：表 `bots`（bot 配置）+ `bindings`（(bot, 群聊[, 话题]) → 会话 id，话题群键为 `bot:chat:thread`）+ `meta`（一次性迁移标记）
...
- 解绑只删密钥凭据；`bindings` 表的 (bot, 群聊[, 话题]) → 会话绑定保留（话题群按话题独立成键），重绑同一应用后继续生效
