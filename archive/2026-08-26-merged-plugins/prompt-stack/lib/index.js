...
function validateConfig(config) {
	if (config.layers.length === 0) throw new Error("prompt-stack: config.layers must define at least one layer");
	const names = /* @__PURE__ */ new Set();
	for (const layer of config.layers) {
		if (layer.name === "model-notes") throw new Error(`prompt-stack: layer name "${MODEL_NOTES_LAYER}" is reserved for the rules' append text`);
...
function apply(ctx, config) {
	...
	const hitRule = (context) => selectRule(config.rules, context.agent?.options?.provider, context.agent?.options?.model);
	const isSubagent = (context) => context.agent?.session?.header?.origin === "subagent";
	for (const layer of config.layers) ctx.systemPrompt.section({
	...
	ctx.on("system-prompt/assemble", async (_assembly, context, next) => {
		...
		const sections = assembled.sections.map((section) => {
			...
			if (layer === void 0) return section;
			return {
				...section,
...
//#endregion
export { Config, MODEL_NOTES_LAYER, apply, inject, name, validateConfig };
