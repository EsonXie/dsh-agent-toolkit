...
# 2. 版本冲突检查（npm 版本不可撤回）
$ErrorActionPreference = 'Continue'  # PS 5.1: EAP=Stop 会把原生命令的 stderr（如 E404）变成终止错误，门禁 2 需容忍 404
...
# 6. 发布验证
Step '发布验证'
$ErrorActionPreference = 'Continue'  # 发布刚完成时 registry 同步可能延迟，E404 的 stderr 在 EAP=Stop 下会变成终止错误
