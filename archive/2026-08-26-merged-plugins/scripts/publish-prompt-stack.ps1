...
<#
.SYNOPSIS
  手动发布 @dsh-agent-toolkit/prompt-stack 到 npm 官方 registry。
.DESCRIPTION
  门禁链：npm 登录检查 → 版本冲突检查 → test → typecheck → pack 内容核查 → 人工确认 → publish → npm view 验证。
  prepack 钩子会在 pack/publish 前自动重跑 bundle（纯 Node 半，无浏览器 bundle）。
.EXAMPLE
  在仓库根目录执行：powershell -File scripts/publish-prompt-stack.ps1
  已在本地跑过测试可跳过：powershell -File scripts/publish-prompt-stack.ps1 -SkipTests
#>
[CmdletBinding()]
param(
  [switch]$SkipTests
...
$PkgName  = '@dsh-agent-toolkit/prompt-stack'
$PkgDir   = Join-Path $PSScriptRoot '..\packages\prompt-stack' | Resolve-Path
...
try {
  ...
  $tgz = "dsh-agent-toolkit-prompt-stack-$version.tgz"
  $entries = tar -tf $tgz
  $required = @(
