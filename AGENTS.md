...
- 已发布（2026-09-02）：`dsh-agent-toolkit@0.2.2` 已发布到官方 registry（新增 `/create-agent` 交互式创建 Agent 命令；引导文本经 `agent.followup` 投递驱动主 Agent，落库复用面板 API 且 PUT 后经列表端点复核取证）。早前：`@dsh-agent-toolkit/token-usage@0.4.1` + `dsh-agent-toolkit@0.2.1`（2026-09-01，修正 README 安装命令补 `--profile`）；token-usage 旧版 0.1.x/0.2.x 已 deprecate（`@dsh-agent-toolkit/prompt-stack`、`@dsh-agent-toolkit/project-bot` 从未发布，无需 deprecate）。
...
- 内存 setup 建会话：`ctx.agents.create` + `setup` 建实时 agent，`setupAgentScope` 在 agentCtx 下 scoped 挂载基础工具行（persona/instructions/shell/fs/fs-search）再叠 persona/tools 白名单（restrict 必须在工具行挂载之后）；委派走 `team_delegate`（一次性），浏览器半渲染委派卡（含 `provider / model` chip：运行中读在途端点、结束后读 presentationMeta）。
