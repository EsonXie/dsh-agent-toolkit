...
- 已发布（2026-09-01）：`@dsh-agent-toolkit/token-usage@0.4.1` + `dsh-agent-toolkit@0.2.1` 已发布到官方 registry（0.2.1/0.4.1 修正 README 安装命令补 `--profile`）；token-usage 旧版 0.1.x/0.2.x 已 deprecate（`@dsh-agent-toolkit/prompt-stack`、`@dsh-agent-toolkit/project-bot` 从未发布，无需 deprecate）。
