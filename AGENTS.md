...
已是 git 仓库（2026-08-18 初始化）；`deepseek-harness/` 被 .gitignore 排除。**2026-09-10 起 checkout 为 0.1.5-rc.1 fresh clone（tag `dsh-v0.1.5-rc.1`，`.git` 在位）；旧 08-18 基线保留为 `deepseek-harness.old-0.1.2/`（0.1.5 兼容性升级全部验证通过后删除）。0.1.5-rc.1 结构变化：内置 agent presets 迁至 `packages/preset/agent-presets/presets/`；`packages/client/runtime`（dsh-client-runtime）裁撤，类型拆到 `api/session-controller` 等；`Context.slots` 声明合并在 `@deepseek-ai/dsh-client-ui-renderer/client`。** 升级进行中台账：[docs/superpowers/plans/2026-09-10-dsh-0.1.5-compat-upgrade.md](docs/superpowers/plans/2026-09-10-dsh-0.1.5-compat-upgrade.md)（含宿主破坏清单、修复状态、飞书流式迁移待办）。
