...
- 已发布（2026-09-03）：`dsh-agent-toolkit@0.2.4` 已发布到官方 registry（BASIC_TOOLS 迁至 tool-scope standing scope（祖先层）+ join 后 restrict，修复带工具白名单 bot 的 unknown global tools）。同日早些：`dsh-agent-toolkit@0.2.3`（修复飞书 bot `/new` "处理失败"：BASIC_TOOLS 引用的 5+1 个宿主工具包补进 devDependencies + 守护测试；入站失败回复携带错误摘要；已读回执 no-op 消噪；**0.2.3 对带工具白名单的 bot 仍会失败**）。早前：`dsh-agent-toolkit@0.2.2`（2026-09-02，新增 `/create-agent` 交互式创建 Agent 命令；引导文本经 `agent.followup` 投递驱动主 Agent，落库复用面板 API 且 PUT 后经列表端点复核取证）；`@dsh-agent-toolkit/token-usage@0.4.1` + `dsh-agent-toolkit@0.2.1`（2026-09-01，修正 README 安装命令补 `--profile`）；token-usage 旧版 0.1.x/0.2.x 已 deprecate（`@dsh-agent-toolkit/prompt-stack`、`@dsh-agent-toolkit/project-bot` 从未发布，无需 deprecate）。
