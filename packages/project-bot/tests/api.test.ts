...
describe('PUT /bots', () => {
  ...
  test('preset：创建落表、更新覆盖、null 清除', async () => {
    ...
    await handler(mockReq('PUT', '/project-bot/api/bots?id=ops', { preset: 'standard' }), update)
    expect(update.status).toBe(200)
    expect(bots.get('ops')).toMatchObject({ preset: 'standard' })

    const clear = mockRes()
    await handler(mockReq('PUT', '/project-bot/api/bots?id=ops', { preset: null }), clear)
    expect(clear.status).toBe(200)
...
describe('GET /presets', () => {
  test('返回名册 preset 清单', async () => {
    const { handler } = harness()
    const res = mockRes()
    await handler(mockReq('GET', '/project-bot/api/presets'), res)
    expect(res.status).toBe(200)
