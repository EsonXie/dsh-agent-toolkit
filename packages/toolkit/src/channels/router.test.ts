...
import { Router, senderSectionText } from './router.ts'
import type { BotRecord } from '../bots/store.ts'
import { bindingKey } from '../bots/store.ts'
...
describe('Router.ensure', () => {
  test('无绑定：create 新 agent 并写绑定，cwd 透传，bot 级 persona/tools 不再注入', async () => {
    ...
    const rt = await router.ensure(fakeBot(), 'oc_1', undefined, reply, 'ou_u1', 'p2p')
  ...
  test('ensure 把发起人 open_id 落进 SessionRuntime', async () => {
    const { router } = setup()
    const rt = await router.ensure(fakeBot(), 'oc_1', undefined, reply, 'ou_initiator_1', 'p2p')
    expect(rt.initiatorOpenId).toBe('ou_initiator_1')
  })

  test('已存在会话直接返回：不改 initiatorOpenId（发起人不变）', async () => {
    const { router } = setup()
    await router.ensure(fakeBot(), 'oc_1', undefined, reply, 'ou_first', 'p2p')
    const reused = await router.ensure(fakeBot(), 'oc_1', undefined, reply, 'ou_other', 'p2p')
  ...
  test('有绑定且进程内有 runtime：直接复用；reply 刷新移交 Inbound 准入后执行', async () => {
    ...
    const first = await router.ensure(fakeBot(), 'oc_1', undefined, reply, 'ou_u1', 'p2p')
    const reply2 = {} as ReplyHandle
    const second = await router.ensure(fakeBot(), 'oc_1', undefined, reply2, 'ou_u1', 'p2p')
  ...
  test('有绑定但进程内无 runtime（重启后）：resume 恢复', async () => {
    ...
    await bindings.set('reviewer', 'oc_1', undefined, 'sess-old')
    const rt = await router.ensure(fakeBot(), 'oc_1', undefined, reply, 'ou_u1', 'p2p')
  ...
  test('retiring 中的会话不复用（重绑窗口）：resume + adopt 重建同一会话，替换后旧 retire 摘除不误删', async () => {
    ...
    first.retiring = true
    const second = await router.ensure(fakeBot(), 'oc_1', undefined, reply, 'ou_u1', 'p2p')
  ...
  test('create 会话以宿主默认模型创建（{provider, model}）', async () => {
    ...
    await router.ensure(fakeBot(), 'oc_1', undefined, reply, 'ou_u1', 'p2p')
  ...
  test('bot 有 agentOptions：原样透传（绑 main 的 bot 自配模型优先于宿主默认）', async () => {
    const { router, created, defaultModel } = setup()
    await router.ensure(fakeBot({ agentOptions: { provider: 'acme', model: 'acme-x' } }), 'oc_1', undefined, reply, 'ou_u1', 'p2p')
...
describe('Router.reset（/new）', () => {
  test('取消旧 agent、清绑定、开新会话', async () => {
    const { router, bindings, created } = setup()
    const old = await router.ensure(fakeBot(), 'oc_1', undefined, reply, 'ou_u1', 'p2p')
    const next = await router.reset(fakeBot(), 'oc_1', undefined, reply, 'ou_u1', 'p2p')
  ...
  test('无旧绑定时直接开新会话', async () => {
    ...
    await router.reset(fakeBot(), 'oc_9', undefined, reply, 'ou_u1', 'p2p')
    expect(created).toHaveLength(1)
  })
  ...
  test('/new：旧会话等 turn 落定后再摘出 sessions（旧卡可 finalize）', async () => {
    ...
    const done = router.reset(fakeBot(), 'oc_1', undefined, reply, 'ou_u1', 'p2p')
    await done
...
describe('Router.ensure agentRef 绑定', () => {
  test('agentRef 指向 main：不注册角色 section、不 restrict，模型取宿主默认，bot 残留 persona/tools 被忽略', async () => {
    ...
    await router.ensure(fakeBot({ agentRef: 'main' }), 'oc_1', undefined, reply, 'ou_u1', 'p2p')
    expect(defaultModel).toHaveBeenCalledOnce()
    expect(created[0].input.agentOptions).toEqual({ provider: 'deepseek', model: 'deepseek-v4' })
    expect(created[0].input.hooks).toEqual({ sections: [GUIDANCE, SENDER] })
  })
  ...
  test('agentRef 指向角色：注册单 persona section + tools.restrict({ allow }) + agentOptions=role.model', async () => {
    ...
    await router.ensure(fakeBot({ agentRef: 'reviewer' }), 'oc_1', undefined, reply, 'ou_u1', 'p2p')
    expect(defaultModel).not.toHaveBeenCalled()
  ...
  test('agentRef 指向角色：记录残留的 agentOptions 不读（角色模型优先）', async () => {
    const { router, created, defaultModel } = setup(undefined, fakeRegistry([MAIN_ROLE, REVIEWER_ROLE]).registry)
    await router.ensure(fakeBot({ agentRef: 'reviewer', agentOptions: { provider: 'acme', model: 'acme-x' } }), 'oc_1', undefined, reply, 'ou_u1', 'p2p')
    expect(defaultModel).not.toHaveBeenCalled()
    expect(created[0].input.agentOptions).toEqual({ provider: 'deepseek', model: 'deepseek-reasoner' })
  })
  ...
  test('agentRef 指向角色且角色未配 model：回退宿主默认模型', async () => {
    ...
    await router.ensure(fakeBot({ agentRef: 'scout' }), 'oc_1', undefined, reply, 'ou_u1', 'p2p')
    expect(defaultModel).toHaveBeenCalledOnce()
    expect(created[0].input.agentOptions).toEqual({ provider: 'deepseek', model: 'deepseek-v4' })
    expect(created[0].input.hooks).toEqual({
      sections: [
  ...
  test('agentRef 指向不存在角色：warn 并降级 main（默认模型，bot 残留 persona/tools 被忽略）', async () => {
    ...
    await router.ensure(fakeBot({ agentRef: 'ghost' }), 'oc_1', undefined, reply, 'ou_u1', 'p2p')
...
describe('Router 发起人提示段', () => {
  ...
  test('/new 重置后新会话仍注入（发起人取当前消息发送人）', async () => {
    const { router, created } = setup()
    await router.ensure(fakeBot(), 'oc_1', undefined, reply, 'ou_u1', 'p2p')
    await router.reset(fakeBot(), 'oc_1', undefined, reply, 'ou_u2', 'p2p')
...
describe('Router.switchTo（/switch）', () => {
  test('boundSessionId：读绑定表（不要求进程内有 runtime）', async () => {
    ...
    await bindings.set('reviewer', 'oc_1', undefined, 'sess-x')
  ...
  test('切到不在内存的会话：resume 接管（装配照常）+ 绑定覆盖不 delete', async () => {
    const { router, bindings, resumed, defaultModel } = setup()
    const old = await router.ensure(fakeBot(), 'oc_1', undefined, reply, 'ou_u1', 'p2p')
    const rt = await router.switchTo(fakeBot(), 'oc_1', undefined, 'sess-target', reply, 'ou_u2', 'p2p')
    expect(resumed).toHaveLength(1)
  ...
  test('切到已在内存的会话：直接复用 runtime，不 resume、initiator 不变', async () => {
    const { router, bindings, sessions, resumed } = setup()
    await router.ensure(fakeBot(), 'oc_1', undefined, reply, 'ou_u1', 'p2p')
    await router.switchTo(fakeBot(), 'oc_1', undefined, 'sess-b', reply, 'ou_u1', 'p2p')
    const rtB = sessions.get('sess-b')!
    const back = await router.switchTo(fakeBot(), 'oc_1', undefined, 'sess-b', reply, 'ou_u2', 'p2p')
  ...
  test('切走的旧 runtime 不 retire：idle 落定且未重新绑定后摘出 sessions', async () => {
    ...
    await router.switchTo(fakeBot(), 'oc_1', undefined, 'sess-target', reply, 'ou_u1', 'p2p')
  ...
  test('摘除窗口内被切回：runtime 保留不误删', async () => {
    ...
    old.agent.whenIdle = () => new Promise<void>((resolve) => { idle = resolve })
    await router.switchTo(fakeBot(), 'oc_1', undefined, 'sess-target', reply, 'ou_u1', 'p2p')
    await router.switchTo(fakeBot(), 'oc_1', undefined, oldId, reply, 'ou_u1', 'p2p')   // 切回（内存复用）
    idle()
    await new Promise((r) => setTimeout(r, 0))
  ...
  test('resume 失败：binding 不变，旧 runtime 不动', async () => {
    ...
    await expect(router.switchTo(fakeBot(), 'oc_1', undefined, 'sess-bad', reply, 'ou_u1', 'p2p')).rejects.toThrow('session corrupted')
  ...
  test('binding 覆盖失败：摘除本次 adopt 的 runtime 并抛错（不留孤儿）', async () => {
    ...
    await expect(router.switchTo(fakeBot(), 'oc_1', undefined, 'sess-target', reply, 'ou_u1', 'p2p')).rejects.toThrow('storage down')
  ...
  test('摘除时 dispose 释放宿主写句柄：之后可 resume 切回（already owned 回归）', async () => {
    ...
    await router.switchTo(fakeBot(), 'oc_1', undefined, oldId, reply, 'ou_u1', 'p2p')
  ...
  test('/new retire：收尾落定后 dispose 释放写句柄（旧会话之后可被 /switch resume 接管）', async () => {
    ...
    const old = await router.ensure(fakeBot(), 'oc_1', undefined, reply, 'ou_u1', 'p2p')
    const oldId = old.sessionId
    await router.reset(fakeBot(), 'oc_1', undefined, reply, 'ou_u1', 'p2p')
  ...
  test('目标在宿主内存存活（web 界面持有写句柄）：接管复用，不 resume（宿主 createOrAdopt 同款）', async () => {
    ...
    const rt = await router.switchTo(fakeBot(), 'oc_1', undefined, 'sess-web', reply, 'ou_u2', 'p2p')
  ...
  test('ensure：绑定会话在宿主存活但不在插件 map（重启窗口外被 web 接管）→ 复用不 resume', async () => {
    ...
    await bindings.set('reviewer', 'oc_1', undefined, 'sess-web')
    const webOwned = fakeAgent('sess-web')
    live.set('sess-web', webOwned)
    const rt = await router.ensure(fakeBot(), 'oc_1', undefined, reply, 'ou_u1', 'p2p')
    expect(resumed).toHaveLength(0)
...
describe('Router 话题键控（threadId）', () => {
  test('同一 chat 不同话题各自建独立会话', async () => {
    const { router, bindings } = setup()
    const rt1 = await router.ensure(fakeBot(), 'oc_1', 'omt_a', reply, 'ou_u1', 'topic')
    const rt2 = await router.ensure(fakeBot(), 'oc_1', 'omt_b', reply, 'ou_u1', 'topic')
    expect(rt1.sessionId).not.toBe(rt2.sessionId)
    expect(rt1.threadId).toBe('omt_a')
    expect(bindings.map.has('reviewer:oc_1:omt_a')).toBe(true)
    expect(router.lookup('reviewer', 'oc_1', 'omt_a')?.sessionId).toBe(rt1.sessionId)
    expect(router.lookup('reviewer', 'oc_1', 'omt_b')?.sessionId).toBe(rt2.sessionId)
  })
  ...
  test('sender 段三形态文案逐字（spec §3.5；p2p 与现文案逐字一致）', () => {
    expect(senderSectionText('feishu', 'ou_1', 'p2p')).toBe('本会话由 feishu 渠道的单聊会话发起。发起人 ID（feishu open_id）：`ou_1`。')
    expect(senderSectionText('feishu', 'ou_1', 'group')).toBe('本会话由 feishu 渠道的群聊会话发起。发起人 ID（feishu open_id）：`ou_1`。')
    expect(senderSectionText('feishu', 'ou_1', 'topic')).toBe('本会话由 feishu 渠道话题群的一个话题发起。该话题绑定独立会话，同一话题群的其他话题互不共享会话。发起人 ID（feishu open_id）：`ou_1`。')
  })
})
